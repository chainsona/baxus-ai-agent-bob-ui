import { Bottle, UserBar } from './api';
...
export function analyzeUserBar(userBar: UserBar) {
  ...
  const profileSummary = generateProfileSummary(bottles, preferredStyles, priceRanges);
...
function generateProfileSummary(
  ...
  const abvValues = bottles.map(bottle => bottle.abv).filter(Boolean) as number[];
  const avgAbv = abvValues.length 
    ? Math.round(abvValues.reduce((sum, abv) => sum + abv, 0) / abvValues.length) 
    : null;
...
export async function generateRecommendations(
  ...
  const candidateBottles = availableBottles.filter(bottle => !userBottleIds.has(bottle.id));
...
function generateRecommendationReason(
  ...
  if (analysis.preferredStyles.includes(bottle.spirit_type)) {
    ...
    reasons.push(`would add variety to your collection with a ${bottle.spirit_type}`);
  }
  ...
  if (bottlePrice <= analysis.priceRanges.average * 0.8) {
    ...
    reasons.push(`is in your typical price range at $${Math.round(bottlePrice)}`);
  }
